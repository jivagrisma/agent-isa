#!/usr/bin/env python3
"""
Script para probar la invocación del modelo Claude en AWS Bedrock.
"""
...
# ID del modelo Claude - usar valor del archivo .env si está disponible
claude_model_id = env_vars.get("BEDROCK_MODEL_CLAUDE", os.environ.get("BEDROCK_MODEL_CLAUDE", "anthropic.claude-3-5-sonnet-20240620-v1:0"))

print(f"Usando modelo Claude: {claude_model_id}")

def invoke_claude(prompt: str, max_tokens: int = 100):
    """
    Invoca el modelo Claude en AWS Bedrock.

    Args:
        prompt: Prompt para el modelo
        max_tokens: Número máximo de tokens a generar
    """
    print(f"Invocando modelo Claude ({claude_model_id}) con prompt: {prompt}")
    ...
    try:
        ...
        # Preparar parámetros para Claude
        request_body = {
...
def main():
    ...
    # Obtener modelo de los argumentos o usar Claude por defecto
    model = "claude"
    if len(sys.argv) > 2:
        model = sys.argv[2]
    ...
    elif model.lower() == "titan":
        invoke_titan_text(prompt)
    else:
        print(f"Modelo no reconocido: {model}")
        print("Modelos disponibles: claude, titan")
